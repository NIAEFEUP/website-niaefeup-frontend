<script lang="ts">
  ...
  const colorList = {
    ...
    reddish: {main: 'bg-taupe-200', hover: 'hover:bg-red-500'}
  };
  const widthList = {
    small: 'w-20',
    medium: 'w-32',
...
<button
  ...
  class="flex {colorList[color]['main']} {colorList[hoverColor][
    'hover'
  ]} justify-center font-raleway font-bold text-rose-950 items-center {widthList[width]} min-w-0 rounded px-4 py-2"
>
  <p class="px-5">{text}</p>
</button>
